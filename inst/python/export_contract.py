...
MINIMAL_EXPORT_PROFILE_ID = "flowjo_minimal_provenance_v1"
...
MINIMAL_PROFILE = "minimal_provenance_v1"
...
ALLOWED_TOP_LEVEL = {
    ...
    "compensation_context", "analysis_mapping",
}
...
ALLOWED_MAPPING = {"dna_channel", "target_channel"}
...
def event_identity_fields(profile: str, acquisition_id: str,
    ...
    return {
        # The minimal profile binds each CSV row to its declared acquisition,
        # but intentionally makes no claim about source-event indices.
        "acquisition_id": [acquisition_id] * len(source_indices) if production or minimal else blank,
...
def resolve_contract_fcs_files(fcs_dir: Path, acquisitions: list[dict[str, Any]],
    ...
    if set(by_sample) != workspace_sample_ids:
        raise ValueError("contract acquisition mapping does not exactly cover selected workspace samples")
    ...
    for sample_id in sorted(workspace_sample_ids):
        ...
        if reference.is_absolute() or ".." in reference.parts:
            raise ValueError("contract source_fcs_reference must be relative and confined")
...
# Compatibility name retained for the pH3 direct-identity exporter/tests.
resolve_production_fcs_files = resolve_contract_fcs_files
...
def manifest_binding_object(manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    binding = {field: manifest[field] for field in fields}
    if "analysis_mapping" in manifest:
        binding["analysis_mapping"] = manifest["analysis_mapping"]
    return binding
...
def validate_manifest_structure(manifest: dict[str, Any]) -> None:
    ...
    if manifest["profile"] not in (PRODUCTION_PROFILE, MINIMAL_PROFILE, LEGACY_PROFILE):
        raise ValueError("manifest profile mismatch")
    if manifest["profile"] == MINIMAL_PROFILE:
        mapping = manifest.get("analysis_mapping")
        if (not isinstance(mapping, dict) or
                set(mapping) != ALLOWED_MAPPING or
                not all(isinstance(mapping[key], str) and mapping[key].strip()
                        for key in ALLOWED_MAPPING)):
            raise ValueError("minimal profile requires an exact DNA/target analysis_mapping")
    ...
    if manifest["profile"] in (PRODUCTION_PROFILE, MINIMAL_PROFILE):
        ...
        if not manifest["requested_populations"] or observed != expected:
            raise ValueError("contract population ledger does not exactly cover requests/acquisitions")
        artifact_scope = {
        ...
        if artifact_scope != expected:
            raise ValueError("contract population artifacts lack exact acquisition linkage")
...
def new_manifest(*, operation_id: str, profile: str, workspace: Path,
    ...
    if profile not in (PRODUCTION_PROFILE, MINIMAL_PROFILE, LEGACY_PROFILE):
        raise ValueError("unknown export profile")
    if profile == MINIMAL_PROFILE and operation_id is not None:
        raise ValueError("minimal profile generates export_operation_id automatically")
    if operation_id is None:
        operation_id = "flowjo-" + uuid.uuid4().hex
    ...
    analysis_mapping = metadata.get("analysis_mapping", {})
    _reject_extra_keys(metadata, ALLOWED_TOP_LEVEL, "metadata")
    if not isinstance(approval, dict) or not isinstance(software, dict) or not isinstance(analysis_mapping, dict):
        raise ValueError("approval, software, and analysis_mapping metadata must be objects")
    _reject_extra_keys(approval, ALLOWED_APPROVAL, "approval")
    _reject_extra_keys(software, ALLOWED_SOFTWARE, "software")
    _reject_extra_keys(analysis_mapping, ALLOWED_MAPPING, "analysis_mapping")
    ...
    if profile in (PRODUCTION_PROFILE, MINIMAL_PROFILE):
        if not isinstance(acquisitions, list) or not acquisitions:
            raise ValueError("minimal contract metadata requires explicit acquisitions")
        for acquisition in acquisitions:
        ...
        _require_text(analysis_mapping, ("dna_channel", "target_channel"), "analysis_mapping")
        _require_text(software, ("supported_flowkit_version",), "software")
        if software["supported_flowkit_version"] != flowkit_version:
            raise ValueError("installed FlowKit version does not equal explicit supported_flowkit_version")
        ...
        if len(sample_ids) != len(set(sample_ids)) or len(acquisition_ids) != len(set(acquisition_ids)):
            raise ValueError("contract acquisition and sample IDs must be unique")
    if profile == PRODUCTION_PROFILE:
        ...
        raise ValueError("only the pH3 direct-identity profile can claim verified direct identity")
    manifest = {
        ...
        "profile": profile,
        "legacy_warning": (
            None if profile in (PRODUCTION_PROFILE, MINIMAL_PROFILE) else
            "LEGACY COUNT-ONLY EXPORT: identity, containment, and geometry linkage are unverified"
        ),
        "export_profile": {
            "id": (EXPORT_PROFILE_ID if profile == PRODUCTION_PROFILE
                   else MINIMAL_EXPORT_PROFILE_ID if profile == MINIMAL_PROFILE
                   else "legacy_count_only_unverified_v1"),
    ...
    if profile == MINIMAL_PROFILE:
        manifest["analysis_mapping"] = analysis_mapping
